...
If pymupdf fails with `ImportError: libstdc++.so.6: cannot open shared object file` (happens on this runtime — the venv Python doesn't have the right native libs), fall back to pdfminer:

```bash
pip install -q pdfminer.six
```

Then use a script file (not inline -c, which breaks on quotes/newlines):
...
Note: always write search scripts to a file rather than using python3 -c — multi-line strings with quotes cause syntax errors in shell.

## Notes

- `web_extract` is always first choice for URLs
- pymupdf is the safe default when it works — instant, no models
- **On this runtime, pymupdf fails** (missing libstdc++) — use pdfminer.six as fallback
